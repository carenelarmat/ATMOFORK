...
!Carene variable for directory containing files indicated by the user
  character(len=MAX_STRING_LEN) :: OUTPUT_FILES_LOC
...
!Carene Ask the user what surface_from_mesher.h to read 
  print *,'Give the the path where the moviedata and the surface_from_mesher.h files are '
  read(*,*) OUTPUT_FILES_LOC
  !open(unit=IIN,file=trim(OUTPUT_FILES)//'surface_from_mesher.h',status='old',action='read',iostat=ier)
  open(unit=IIN,file=trim(OUTPUT_FILES_LOC)//'surface_from_mesher.h',status='old',action='read',iostat=ier)
  if (ier /= 0) then
    !print *,'error opening file: ',trim(OUTPUT_FILES)//'surface_from_mesher.h'
    print *,'error opening file: ',trim(OUTPUT_FILES_LOC)//'surface_from_mesher.h'
    print *
  ...
  i = index(line,'=')
  if (i == 0) stop 'error reading in NSPEC_SURFACE_EXT_MESH from file surface_from_mesher.h'
...
! loop on all the time steps in the range entered
  do it = it1,it2
    ...
    if (mod(it,NTSTEP_BETWEEN_FRAMES) == 0 .or. plot_shaking_map) then
      ...
      open(unit=IOUT,file=trim(OUTPUT_FILES_LOC)//trim(outputname),status='old', &
            action='read',form='unformatted',iostat=ier)
      if (ier /= 0) then
        !print *,'error: ',trim(OUTPUT_FILES)//trim(outputname)
        print *,'error: ',trim(OUTPUT_FILES_LOC)//trim(outputname)
      ...
      if (plot_shaking_map) then
        ...
        if (USE_OPENDX) then
          ...
          open(unit=11,file=trim(OUTPUT_FILES_LOC)//outputname,status='unknown')
...
enddo ! it
  ...
  if (USE_OPENDX) print *, 'DX files are stored in ', trim(OUTPUT_FILES_LOC), '/DX_*.dx'
  if (USE_AVS) print *, 'AVS files are stored in ', trim(OUTPUT_FILES_LOC), '/AVS_*.inp'
  if (USE_GMT) print *, 'GMT files are stored in ', trim(OUTPUT_FILES_LOC), '/gmt_*.xyz'
  print *
